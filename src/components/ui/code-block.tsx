...
import { forwardRef, type HTMLAttributes, useState, useEffect, useRef } from "react";
...
const CodeBlock = forwardRef<HTMLDivElement, CodeBlockProps>(
  ({ className, language, children, ...props }, ref) => {
    ...
    useEffect(() => {
      ...
      if (codeContent.trim() !== '') {
        try {
          ...
          setHighlightedHtml(hljs.highlight(codeContent, { language: 'plaintext' }).value);
        }
      } else {
        setHighlightedHtml('');
      }
    ...
    // Ensure text direction is LTR for code editing
    useEffect(() => {
      if (codeRef.current) {
        codeRef.current.style.direction = 'ltr';
        codeRef.current.style.textAlign = 'left';
      }
    }, []);
...
// Re-export all the sub-components from the original file
...
const CodeHeader = forwardRef<HTMLDivElement, CodeHeaderProps>(
  ({ className, ...props }, ref) => {
    return (
      <div
        className={codeHeaderVariants({ className })}
        ref={ref}
        {...props}
      />
    );
...
const CodeBody = forwardRef<HTMLDivElement, CodeBodyProps>(
  ({ className, ...props }, ref) => {
    return (
      <div
        className={codeBodyVariants({ className })}
        ref={ref}
        {...props}
      />
    );
